# Here, we are doing feature engineering and model building in model building as the dvc is not able to handle the train file of 1 GB from feature engineering after applying TF-IDF. Generally feature engineering are used in different file.

# We can perform hyperparameter tunning during model building but we are not doing it because error may happen during hyperparameter tunning. Thus, we are performing hyperparameter tunning in the Experimentation stage and put the final hyperparameters into params.yaml for dvc.yaml. To differentiate experimentation and production code (both are valid and common in practice).

# fetch the data from data/processed > apply algorithm like TF-IDF with trigram and max_features 10000 > model building with LightGBM with Hyperparameters we get from the EDA/Hyperparameter tunning > saving model into .pkl file and Vectorizer into .pkl file

from sklearn.feature_extraction.text import TfidfVectorizer
import logging, joblib, yaml, os
